...
// 看下面这一句，这是关键一句。得到的 request 对象可以直接按照
...
describe("test/app.test.js", function () {
    ...
    it("should return 55 when n is 10", function (done) {
        // 之所以这个测试用例的 function 需要接受一个done函数，是因为我们此时内容设计了异步调用，
        // 而mocha是无法感知异步调用完成的。所以我们主动接受他提供的done函数，在测试完成时，
        // 自行调用一下，以示结束。
        // mocha可以感知到我们测试函数是否接受 done参数。js中，function对象是有长度的，
        // 他的长度由他的参数数量决定 (function(a,b,c,d)).length===4 .
        // 所以 mocha 通知我们测试函数的长度就可以确定我们是否是异步测试。

        // query 方法用来传递querystring,
        // end 方法用来传递 body,
        // 它们都可以传 Object 对象进去。
        // 在这里，相当于访问的是 /fib?n-=10
        request.get("/fib").query({n: 10}).end(function (err, res) {
            // 由于 http 返回的是 String, 所以我们要传入 '55' .
            res.text.should.equal('55');

            // done(err) 这种用法写起法很鸡肋，是因为偷懒不想测err的值
            // 如果勤快点，这里应该这样写
